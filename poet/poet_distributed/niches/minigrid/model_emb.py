...
class Model:
    ...
    def __init__(self, game):
        ...
        self.input_size = 9*self.EMBEDDING_SIZE#3#game.input_size
        ...
        self.bias_log_std = []
        self.bias_std = []
        self.embedding = np.random.random(size=(8, self.EMBEDDING_SIZE))
        self.param_count = 8*self.EMBEDDING_SIZE
        idx = 0
    ...
    def set_model_params(self, model_params):
        pointer = 0
        self.embedding = np.array(model_params[pointer:pointer+(8*self.EMBEDDING_SIZE)]).reshape(8, self.EMBEDDING_SIZE)
        pointer += (8*self.EMBEDDING_SIZE)
